...
from utils.measure import (calculate_distance, degrees_to_radians,
                           get_square_distance_map, rotate_point_2d,
                           trilateration_3d)
...
class SocketManager:
    def __init__(self, plt_datas):
        ...
        self.clientMap = dict()
        self.devices = dict()
        self.segmentModal = load_model(
            download_model(BodyPixModelPaths.MOBILENET_FLOAT_75_STRIDE_16)
        )
    ...
    def _socketInit(self):
        ...
        self.sio_app = socketio.ASGIApp(
            socketio_server=self.sio_server, socketio_path="/"
        )
        ...
        async def connect(sid, environ, auth):
            ...
            self.camera_position = np.matmul(
                -1 * cv2.Rodrigues(self.rvecs)[0].T, self.tvecs
            )
        ...
        async def on_message(sid, data):
            ...
            if len(heads) > 0:
                ...
                for head in heads:
                    ...
                    cv2.circle(
                        added_image, (int(self.c_x), int(self.c_y)), 1, (0, 0, 255), 3
                    )
                    ...
                    cv2.putText(
                        added_image,
                        "{},{}".format(rotated_cameraCoord[0], rotated_cameraCoord[2]),
                        (u, v + h + 90),
                        0,
                        5e-3 * 130,
                        (0, 0, 255),
                    ...
                    for device in self.devices.keys():
                        device_coord = self.devices[device]["coordinate"][
                            "point_on_line"
                        ][0:2]
                        ...
                        if a < THRESHOLD_DISTANCE:
                            ...
                            await self.sio_server.emit(
                                "turn_on", "turn_on", namespace="/temp"
                            )
                        else:
                            print("off")
                            await self.sio_server.emit(
                                "turn_off", "turn_off", namespace="/temp"
                            )
        ...
        async def on_message(sid, data):
            ...
            if not all(value == 0 for value in distances):
                self.devices[sid]["coordinate"] = trilateration_3d(
                    point1, point2, point3, *distances
                )

            for device in self.devices.keys():
                if "coordinate" not in self.devices[device]:
                    break
                device_coord = np.array(
                    self.devices[device]["coordinate"]["point_on_line"][0:2]
                )
        ...
        async def on_message(sid, data):
            ...
            if conditionSatisfied:
                while True:
                    ...
                    await self.sio_server.emit(
                        "calibrate", data2, to=pair[1], namespace="/iot_calibrate"
                    )
                    ...
                    await self.sio_server.emit(
                        "calibrate", data1, to=pair[0], namespace="/iot_calibrate"
                    )
                    await self.IOT_Calibrater.wait_received()
                    print("received")

                    await self.sio_server.emit(
                        "end_calibrate", to=pair[0], namespace="/iot_calibrate"
                    )
                    await self.sio_server.emit(
                        "end_calibrate", to=pair[1], namespace="/iot_calibrate"
                    )
    ...
    async def boardcast_all_devices(self):
        b = [
            self.IOT_Calibrater.sidDict[sid]["delay"]
            for sid in self.IOT_Calibrater.sidDict.keys()
        ]
        ...
        for index, sid in list(enumerate(self.IOT_Calibrater.sidDict.keys())):
            ...
            c["delays"] = b
            await self.sio_server.emit(
                "terminate_calibrate", c, to=sid, namespace="/iot_calibrate"
            )
